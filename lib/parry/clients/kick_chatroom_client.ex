defmodule Parry.Clients.KickChatroomClient do
  use WebSockex
  require Logger
  ...
  defp normalize({:ok, data}) do
    ...
    Parry.Chat.Message.changeset_from_event(%Parry.Chat.Message{}, data)
  end

  defp write(changeset) do
    Logger.debug("Writing event")
    Parry.Repo.insert!(changeset)
  end

  defp notify(record) do
    Logger.debug("Broadcasting insert to chatroom-#{record.external_chatroom_id} for #{record.id}")
    ParryWeb.Endpoint.broadcast("chatroom-#{record.external_chatroom_id}", "insert", record.id)
  end
end
